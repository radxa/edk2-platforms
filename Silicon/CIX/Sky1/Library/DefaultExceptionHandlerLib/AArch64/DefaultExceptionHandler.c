  ...
  )
{
  ...
  rlog_printf(LOGLEVEL_INFO,"%s Exception at 0x%016lx\n", gExceptionTypeString[ExceptionType], SystemContext.SystemContextAArch64->ELR);
#endif
...
#ifdef CONFIG_RLOG_ENABLE
  //rlog_printf(LOGLEVEL_INFO,Buffer);
  rlog_printf(LOGLEVEL_INFO,"PC 0x%012lx (0x%012lx+0x%08x) [ 0] %s\n",
      SystemContext.SystemContextAArch64->ELR,
      ImageBase,
      SystemContext.SystemContextAArch64->ELR - ImageBase,
      BaseName (Pdb));
...
#ifdef CONFIG_RLOG_ENABLE
  rlog_printf(LOGLEVEL_INFO,"PC 0x%012lx\n", SystemContext.SystemContextAArch64->ELR);
#endif
  }
...
#ifdef CONFIG_RLOG_ENABLE
  rlog_printf(LOGLEVEL_INFO,"PC 0x%012lx (0x%012lx+0x%08x) [% 2d] %s\n",
          Fp[1],
          ImageBase,
          Fp[1] - ImageBase,
          Idx,
  ...
  SET_REBOOT_REASON(DefaultException);
