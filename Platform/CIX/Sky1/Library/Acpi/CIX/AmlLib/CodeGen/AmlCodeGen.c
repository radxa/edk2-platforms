  ...
  )
{
  ...
  AcpiHeader.CreatorId       = EFI_ACPI_CREATOR_ID;
  AcpiHeader.CreatorRevision = EFI_ACPI_CREATOR_REVISION;
  ...
  )
{
  ...
  AML_OBJECT_NODE_HANDLE  PsdPackageNodeLevel1;
  AML_OBJECT_NODE_HANDLE  PsdPackageNodeLevel2;

  Status = AmlCodeGenNamePackage ("_PSD", ParentNode, &PsdNode);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }
  ...
  // which is the 2nd fixed argument (i.e. index 1).
  PsdPackageNodeLevel1 = (AML_OBJECT_NODE_HANDLE)AmlGetFixedArgument (
                                                   PsdNode,
                                                   EAmlParseIndexTerm1
                                                   );
  if ((PsdPackageNodeLevel1 == NULL)                                              ||
      (AmlGetNodeType ((AML_NODE_HANDLE)PsdPackageNodeLevel1) != EAmlNodeObject)  ||
      (!AmlNodeHasOpCode (PsdPackageNodeLevel1, AML_PACKAGE_OP, 0)))
  {
    ASSERT (0);
    Status = EFI_INVALID_PARAMETER;
    goto error_handler;
  ...
  Status = AmlCodeGenPackage (&PsdPackageNodeLevel2);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }

  Status = AmlAddRegisterOrIntegerToPackage (NULL, NumEntries, PsdPackageNodeLevel2);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }

  Status = AmlAddRegisterOrIntegerToPackage (NULL, Revision, PsdPackageNodeLevel2);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }

  Status = AmlAddRegisterOrIntegerToPackage (NULL, Domain, PsdPackageNodeLevel2);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }

  Status = AmlAddRegisterOrIntegerToPackage (NULL, CoordType, PsdPackageNodeLevel2);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }

  Status = AmlAddRegisterOrIntegerToPackage (NULL, NumProcessors, PsdPackageNodeLevel2);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }

  Status = LinkNode (PsdPackageNodeLevel2, (AML_NODE_HEADER  *)PsdPackageNodeLevel1, NewCpcNode);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    goto error_handler;
  }
...
error_handler:
  ...
  AmlDeleteTree ((AML_NODE_HANDLE)PsdPackageNodeLevel1);
  AmlDeleteTree ((AML_NODE_HANDLE)PsdPackageNodeLevel2);
  return Status;
}

/** Adds a name string to the package

  @ingroup CodeGenApis

  @param [in]  NameString    The name string to add to the package.
                             Must be a NULL-terminated ASL NameString
                             e.g.: "CPU0", "\SB_.CPU0", etc.
                             This input string is copied.
  @param [in]  PackageNode  Package to add value to
