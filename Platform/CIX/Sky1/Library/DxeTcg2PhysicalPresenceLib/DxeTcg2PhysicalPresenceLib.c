...
//Corresponds to the TPMC in Dtpm acpi table
typedef struct _TPMC
{
  UINT32 PPIO; // current PPI request
  UINT32 PPIR; // last PPI request
  UINT32 PPIS; // last PPI request Status
}TPMC;
  ...
  )
{
  ...
  /*
    ************************************************************************
      Can not request to lock after OnreadytoBoot,
      so Execute the lock process in advance as a new module: TPMVariableDxe
    ************************************************************************
  //
  ...
  */
  ...
  TPMC* Tpmc = (TPMC*) FixedPcdGet32(PcdTPMPPIMemoryBase);
  Tpmc->PPIO = 0;
  Tpmc->PPIR = TcgPpData.LastPPRequest;
  Tpmc->PPIS = TcgPpData.PPResponse;
