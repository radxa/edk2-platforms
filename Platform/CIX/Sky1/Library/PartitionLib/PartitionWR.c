...
EFI_STATUS ReadFromPartition (EFI_BLOCK_IO_PROTOCOL *BlockIo,
                              ...
                              IN UINT64 *OutNumRead)
{
  ...
  if (*OutNumRead < NumBytes && (NumBytes + Offset) % PageSize != 0) {
    ...
    if (Status == EFI_SUCCESS) {
      CopyMem (Buffer + (NumBytes - LastPageReadSize), Page,
               LastPageReadSize);
...
EFI_STATUS ReadFromPartitionByName (IN CHAR16 *PartitionName, IN INT64 Offset, IN UINT64 Size, IN VOID *Buffer)
{
  EFI_STATUS Status;
  ...
  DEBUG ((EFI_D_INFO, "Read buffer header: 0x%x 0x%x 0x%x 0x%x 0x%x\n",
...
EFI_STATUS
WriteToPartition (EFI_BLOCK_IO_PROTOCOL *BlockIo,
                   IN EFI_HANDLE *Handle,
  ...
  if (Offset % PageSize != 0) {
    ...
    if (StartPageWriteSize <= 0 || StartPageWriteOffset >= PageSize ||
      ...
      DEBUG ((EFI_D_ERROR,
              ...
              StartBlock, StartPageWriteOffset, StartPageWriteSize));
      Status = EFI_VOLUME_FULL;
      goto out;
    }
...
EFI_STATUS WriteToPartitionByName (IN CHAR16 *PartitionName, IN INT64 Offset, IN UINT64 Size, IN VOID *Buffer)
{
  EFI_STATUS Status;
  EFI_BLOCK_IO_PROTOCOL *BlockIo = NULL;
  EFI_HANDLE *Handle = NULL;
  ...
  DEBUG ((EFI_D_INFO, "Buffer header: 0x%x 0x%x 0x%x 0x%x 0x%x\n",
                         ((unsigned char *)Buffer)[0],
                         ((unsigned char *)Buffer)[1],
                         ((unsigned char *)Buffer)[2],
                         ((unsigned char *)Buffer)[3],
...
EFI_STATUS ErasePartition (EFI_BLOCK_IO_PROTOCOL *BlockIo, EFI_HANDLE *Handle)
{
  ...
  Status = gBS->HandleProtocol (Handle, &gEfiPartitionRecordGuid, (VOID **)&PartEntry);
  if (Status != EFI_SUCCESS) {
  ...
  Status = EraseProt->EraseBlocks (EraseProt, BlockIo->Media->MediaId, PartEntry->StartingLBA,
...
EFI_STATUS ErasePartitionByName (IN CHAR16 *PartitionName)
{
  EFI_STATUS Status;
  EFI_BLOCK_IO_PROTOCOL *BlockIo = NULL;
  EFI_HANDLE *Handle = NULL;
